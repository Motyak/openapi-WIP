...
class ArtistDiscographyAlbumObject(BaseModel):
    ...
    album_group: StrictStr = Field(..., description="This field describes the relationship between the artist and the album. ")
    __properties = ["album_type", "total_tracks", "available_markets", "external_urls", "href", "id", "images", "name", "release_date", "release_date_precision", "restrictions", "type", "uri", "artists", "album_group"]
    ...
    @validator('album_group')
    def album_group_validate_enum(cls, value):
        """Validates the enum"""
        if value not in ('album', 'single', 'compilation', 'appears_on'):
            raise ValueError("must be one of enum values ('album', 'single', 'compilation', 'appears_on')")
        return value
    ...
    @classmethod
    def from_json(cls, json_str: str) -> ArtistDiscographyAlbumObject:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> ArtistDiscographyAlbumObject:
        """Create an instance of ArtistDiscographyAlbumObject from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return ArtistDiscographyAlbumObject.parse_obj(obj)

        _obj = ArtistDiscographyAlbumObject.parse_obj({
            ...
            "album_group": obj.get("album_group")
        })
        return _obj
